#####Conexiones Aula####
c1 <- c('user="jvelazhe",host="50.57.15.219",port=3306,password="68iCQ2deKn",dbname="moodle_aula01n23")')
c2<- c('user="jvelazhe",host="198.101.170.172",port=13302,password="68iCQ2deKn",dbname="moodle_aula02n23")')
c3<- c('user="jvelazhe",host="198.101.170.172",port=13303,password="68iCQ2deKn",dbname="moodle_aula03n23")')
c4<-c('user="jvelazhe",host="50.57.15.219",port=3306,password="68iCQ2deKn",dbname="moodle_aula04n23")')
c5<-c('user="jvelazhe",host="198.101.170.172",port=13302,password="68iCQ2deKn",dbname="moodle_aula05n23")')
c6<-c('user="jvelazhe",host="198.101.170.172",port=13303,password="68iCQ2deKn",dbname="moodle_aula06n23")')
c7<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula07n23")')
c8<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula08n23")')
c9<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula09n23")')
c10<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula10n23")')
c12<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula12")')
c13<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula13")')
c14<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula14")')
c11<-c('user="jvelazhe",host="162.209.35.80",port=3306,password="68iCQ2deKn",dbname="moodle_aula11n23")')
c15<-c('user="jvelazhe",host="161.47.44.102",port=3306,password="68iCQ2deKn",dbname="moodle_aula15")')
...
where mf.name like '%Foro _'  and  mc.category in"
queryforo2 = "order by mc.shortname"



queryforopresentacion="SELECT 
mc.shortname AS clave,
mc.fullname AS asignatura,
mf.name AS foro,
mfd.name AS tema_foro,
...
mfp.parent AS parent_post,
mfp.discussion as linkforo,
SUBSTRING_INDEX(SUBSTRING_INDEX(mfp.message, 'https://www.canva.com/design/', -1),'\"',1)AS enlace_completo
FROM mdl_forum mf 
LEFT JOIN mdl_forum_discussions mfd ON mfd.forum = mf.id 
LEFT JOIN mdl_forum_posts mfp ON  mfp.discussion = mfd.id
LEFT JOIN mdl_course mc ON mc.id = mf.course
LEFT JOIN mdl_groups mg ON mg.id = mfd.groupid 
LEFT JOIN mdl_user mu ON  mu.id = mfp.userid
where  mf.name like '%presentación%' 
and mc.category in"

queryforoconsultas="SELECT 
mc.shortname AS clave,
mc.fullname AS asignatura,
mf.name AS foro,
mfd.name AS tema_foro,
...
LEFT JOIN mdl_groups mg ON mg.id = mfd.groupid 
LEFT JOIN mdl_user mu ON  mu.id = mfp.userid
where  mf.name like '%Espacio%'
and mc.category in"


queryforopresentacionp="SELECT 
mc.shortname AS clave,
mc.fullname AS asignatura,
mf.name AS foro,
mfd.name AS tema_foro,
...
LEFT JOIN mdl_groups mg ON mg.id = mfd.groupid 
LEFT JOIN mdl_user mu ON  mu.id = mfp.userid
where mfd.name like '%Foro _'  and  mc.category in"       

queryforoprating ="SELECT itemid,rating FROM mdl_rating"
...
SELECT 
    ...
    from_unixtime(timecreated,'%Y-%m-%d %k:%i') as 'Fecha.envio'
FROM 
    mdl_messages mm
WHERE 
    mm.timecreated >= UNIX_TIMESTAMP('2023-08-21 00:00:00') 
