...
Rep <- leer(c("Asignación Posgrados","Bases$"),sheet="Base") %>% select("matricula"=Matricula,Campus,`NI-Q`,Tipo,Modalidad,Estado) %>%
  filter(Estado %in% c("Activo","FUTUROS","MM","MM (Activo)","Egresado")) %>%
  left_join(Base,by="matricula") %>% rename("Bim"=B.ingreso) %>%
  filter(is.na(Formación)) %>% filter(grepl("^(B|Q)|NI (29 MAY|03 JUL|31 JUL|28 AGO|25 SEP|23 OCT|20 NOV|08 ENE)",`NI-Q`)) %>%
