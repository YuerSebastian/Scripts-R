...
c1 <- "//Tut-095/reportes"; c2 <- "C:/Users/jsalinba/Documents/Reportes"
diremail(c1,"jsalinba@utel.edu.mx")
Drive <- leer(c("1dEeJqXn944LaVSvDGAq2awfaQdDDhHveWmDGCqFcb_c","gsheet.ID"),sheet="Asignación")
#####--------------Inicio--------------#####
#Leyendo base
Base <- leer(c("AsignacionEjecutiva_Senior","Posgrados$"),sheet="Asignacion",skip=1) %>% mutate(Matricula=if_else(nchar(Matricula)<9,paste("0",Matricula,sep=""),Matricula))%>%
...
Rep <- leer(c("1dEeJqXn944LaVSvDGAq2awfaQdDDhHveWmDGCqFcb_c","gsheet.ID"),sheet="Reasignación")%>%
  ...
  arrange(desc(Fecha)) %>% filter(!duplicated(MATPROG),Base=="Ejecutivos y Senior") %>% select(2:8)%>%
