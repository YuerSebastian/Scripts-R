...
c <- 1; f1 <- ymd("2022-05-02"); f2 <- f1 + days(6)
while (f1 <= Sys.Date()) {
...
Rep <- leer(c("Estatus_General_Alumno","SIR$"),
  ...
  arrange(desc(`Fecha Estatus`),desc(`Fecha PISE`),desc(`Fecha Inicio`)) %>% filter(!duplicated(matricula)) %>% select(matricula,grep("Clave",names(.)))
