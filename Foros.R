sapply(c("dplyr","tidyr","lubridate","readxl","vroom","data.table","googledrive","googlesheets4","stringi","stringr","fs"),require,character.only=T)
#####-----Descargas-----#####
...
Base <- vroom("D:/Trabajo/Bases/Drive/Foros_Posgrapresentacion.csv", ",", col_types = cols(.default = "c"), locale = locale(encoding = "UTF-8")) %>%
  filter(grepl("presen|bienv",asunto,ignore.case = T),!grepl("semana|unidad|open class",asunto,ignore.case = T)) %>%
  ...
  mutate(tipo_mat = if_else(!is.na(matricula_participante),if_else(grepl("^19",matricula_participante),"Do","Al"),"SM"),
         ...
         Apertura = if_else(grepl("presen|bienv",tema_foro,ignore.case = T),"Apertura_Correcta","Apertura_Incorrecta"),
  ...
  unite(ID,clave,grupo,remove = F)
#-----Respuestas-----#
#Rep <- filter(Base,grepl("Re:",asunto),tipo_mat=="Do") %>% unite(ID,clave,grupo) %>% count(ID,name="Respuestas_Docente")
Rep <- filter(Base,tipo_mat=="Do") %>% unite(ID,clave,grupo) %>% count(link_foro,name="Respuestas_Docente")
Base <- left_join(Base,Rep,"link_foro")
#Rep <- filter(Base,grepl("Re:",asunto),tipo_mat=="Al") %>% unite(ID,clave,grupo) %>% count(ID,name="Respuestas_Estudiantes")
Rep <- filter(Base,tipo_mat=="Al") %>% unite(ID,clave,grupo) %>% count(link_foro,name="Respuestas_Estudiantes")
Base <- left_join(Base,Rep,"link_foro")
...
#-----Operación Mae y Doc-----#
Rep <- list(Mae1 = read_sheet("18kyoNbSewZ5lgpoUyI-qs4Ta50Wde8HA5FAZp_fK2MM",sheet = "MAE MB024_B",col_types = "c"),
            Mae2 = read_sheet("18kyoNbSewZ5lgpoUyI-qs4Ta50Wde8HA5FAZp_fK2MM",sheet = "MAE MB024_0",col_types = "c"),
            Doc = read_sheet("1gMij94xq1jiPq8HCn5kMnV9wZrDCB3yjU8jfDu6VRu0",sheet = "Doctorado Septiembre 2023",col_types = "c")) %>%
  lapply(select,"Clave"=Shortname,Grupo,Asignatura,"Mat_Docente"=Clave_prof,"Docente"=Nombre,Gestor,"BLOQUE"=2) %>%
  bind_rows() %>% filter(!grepl("seminario|alianzas|google",Asignatura,ignore.case = T)) %>% unite(ID,Clave,Grupo,remove = F) %>% filter(!is.na(Clave)) %>%
  left_join(Base,"ID")
Base <- Rep
