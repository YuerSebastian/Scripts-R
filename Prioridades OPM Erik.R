#install.packages("D:/Licenciatura/MultiLEDS_1.2.2.tar.gz")
sapply(c("MultiLEDS","dplyr","tidyr"),library,character.only=T)
diremail("//9Z1XBH2/Licenciatura/OPM")
#####-----------Materias-----------#####
Rep <- leer(c("Ponderaciones_Prioridades","OPM$"),sheet="Prioridades1")
mats <- filter(Rep,!duplicated(Clave)) %>% .[["Clave"]] #Se debe crear for completo
Base <- leer(c("sabana_opm","OPM$")) %>% mutate_at(c("Semana","calificacion"),~as.numeric(.)) %>% mutate(Prioridad="Sin prioridad") %>%
...
escribir(Base,c("Prioridades OPM.csv","Licenciatura/OPM$"))
