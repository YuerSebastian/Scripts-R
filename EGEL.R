...
shell("D:/Scripts/Python/EGEL.py")
diremail("D:/Trabajo","jsalinba@utel.edu.mx")
#####-----------Uniendo Calificaciones finales y de actividades-----------#####
Rep <- leer(c("Calif_exam EGEL","EGEL/Descargas$")) %>% group_by(usuario,asignatura,actividad) %>% mutate(calificación=as.numeric(calificación)) %>% 
  ...
  select(id,actividad,comenzado,completado,requerido,calificación,intento) %>% left_join(
    leer(c("Calif_final EGEL","EGEL/Descargas$")) %>% select(matricula,asignatura,`ultimo acceso aula`,"Calificación Final"=calificacion)%>%
      unite(id,matricula,asignatura,sep="_"),by="id")
#####-----------Cruzando con base principal Acceso status-----------#####
Base <- leer(c("Acc_status EGEL","EGEL/Descargas$")) %>% unite(id,matricula,asignatura,sep="_",remove=F) %>% left_join(Rep,by="id") %>% mutate_all(~as.character(.))%>%
