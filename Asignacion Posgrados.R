...
diremail(c2,"jsalinba@utel.edu.mx",c("10.1.46.182",3306,"jsalinba","251708Bi","crm"))
#Consulta MySQL
Rep <- leer(c("select matricula_c,email_address,id,fecha_creacion from rep_alumnos","msql")) %>% rename("Matricula"=matricula_c,"Correo"=email_address,"ID_CRM"=id)%>%
  mutate(Matricula=as.numeric(Matricula),fecha_creacion=ymd_hms(fecha_creacion)) %>% arrange(desc(fecha_creacion)) %>% filter(!is.na(Matricula)) %>% filter(!duplicated(Matricula))
escribir(Rep,c("IDCRM.csv","CRM$"))
#Subiendo a drive
googledrive::drive_put("C:/Users/jsalinba/Documents/Reportes/Bases/Originales/CRM/IDCRM.csv","Contactacion/")
#Descargas
drive_sd("des",c("Contactacion/Conteos_Interacciones.csv","CRM$")); drive_sd("des",c("Reports/acc_status.csv","Otros$"))
cols <- c("Tutor Anterior","Tutor","Supervisor","Turno","NR Autogestionable","Fecha Inscripción","Mora","Docs","Avance","Promedio","Aprobadas","Reprobadas","En Curso","Whatsapp",
          "Contacto Telefono","Colaboradores","CICLO JULIO","TUTOR NIVEL","OBSERVACIONES","TELEFONOS","Clasificación anterior")
#####----------------Paquetes Dinamicos---------------#####
Rep <- leer(c("Paquetes Dinámicos","SIR$"),col_select=c(MATRICULA,"Tipo"=SESION_EJECUTIVA,FECHA_DE_CREACION,ESTATUS_ALUMNO)) %>%
  filter(!is.na(Tipo) & ESTATUS_ALUMNO!="CANCELADA")%>%
  mutate(FECHA_DE_CREACION=dmy_hms(FECHA_DE_CREACION),Tipo="Sesion_Ejecutiva") %>% arrange(desc(FECHA_DE_CREACION)) %>% filter(!duplicated(MATRICULA))
escribir(Rep,c("Sesiones Ejecutivas.csv","auxiliares$"))
#####----------------NIRI---------------#####
Base <- leer(c("Inscritos NIRI","SIR$"),col_select=c("Matricula"=MATRICULA,"Nombre"=ESTUDIANTE,CAMPUS,"Canal"=CANAL_FINAL,NIVEL,"Programa"=PROGRAMA,ESTATUS,DECISION,
  ...
  mutate_at(grep("^f|^f\\.|inicio",names(.),ignore.case = T),~dmy(.)) %>% arrange(desc(F.Decisón),desc(Inicio_Curso))%>%
  mutate(`Clave Programa`=substring(Programa,1,10),Programa=substring(Programa,12))%>%
...
#Filtrando solo Online y quitando sesiones ejecutivas, filtra de manera que no sea necesario guardar en otra variable el dataframe temporal
diremail(c2)
Base <- filter(Base,Tipo=="Online") %>% filter(!(Matricula %in% leer(c("Sesiones Ejecutivas","auxiliares$"))$MATRICULA)) %>%
  left_join(leer(c("Campus","auxiliares$")),by="CAMPUS")
#####----------------Base anterior,Nuevos----------------#####
diremail(c2)
Rep <- leer(c("Asignación Posgrados","Bases$"),sheet="Base") %>% filter(Base=="Online") %>% select(-Base) %>%
...
#####----------------Estatus General----------------#####
#diremail(c2); 
x <- c("MATPROG","Estado Argos","Fecha cambio de Edo","Entidad")
...
Base <- Base[Rep] %>% mutate_all(~as.character(.)) %>% mutate_all(~replace_na(.,"_")) %>% mutate(Base="Online")
...
data.table::fwrite(Base,"C:/Users/jsalinba/Documents/Reportes/Bases/Principales/Asignación Posgrados.csv",bom = T,na = "_")

#escribir(Base,c("Asignación Posgrados.csv","Principales$"),cod = "LATIN1")

source("C:/Users/jsalinba/Documents/Reportes/Scripts/R/Asignacion Posgrados Ejec.R",encoding = "LATIN1")
